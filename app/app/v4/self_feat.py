...
class SelfFeat:
    def __init__(self):
        ...
        self.seq_group_add = 'validADD'
        self.seq_bins_add = [0,1,5,20,90,np.inf]
        ...
        self.seq_group_app1='type'
        self.seq_bins_app1 = [0,1,3,5,7,15,30,60,90,120,180,np.inf]
        ...
        self.seq_group_app2='type'
        self.seq_bins_app2 = [0,7,20,40,100,250,356,500,np.inf]
    ...
    def __cal_seq_varbins(self,df: pd.DataFrame, seq_bins: List[int], seq_var: str, seq_group: str = None):
        ...
        for col in cols:
            if col == 'sum_total':
                ...
                # 组别切片比率 与 组别平均比率 比
                avg_rate = max(result_df['sum_' + col]) / max(max(result_df['sum_total']), 1)
                result_df[col + '_lift'] = result_df.apply(
                    lambda row: round(row[col + '_rate'] / max(avg_rate, 0.00001), 2
                ...
                else round(row[col] / max(max(result_df[col]), 1), 2), axis=1)
                # 组别增率 与 总增率差异
