...
class TF2Vec:
    ...
    def __pre(self, app_df: pd.DataFrame):
        ...
        app_df['packageName'] = app_df['packageName'].apply(lambda x:x.replace(' ',''))

        cols = ['updateAPP_tday', 'install_updateday', 'installAPP_tday']
        app_df[cols] = app_df[cols].astype('int')

        # 根据安装日期排序,从早到晚
        app_df.sort_values(by=['loan_app_id', 'installAPP_tday'], ignore_index=True, inplace=True, ascending=False)
        return app_df

    def __tf_idf_feat(self, app_df: pd.DataFrame):
        # app_df = self.__pre(app_df)
        app_tf_df = pd.merge(app_df, self.app_tf_base, on='packageName')
        ...
        if fill_col:
            app_tf_df.loc[:,fill_col] = 0
        return app_tf_df

    def __app2vec_feat(self, app_df: pd.DataFrame):
        # app_df = self.__pre(app_df)
        # 创建预测语料
        app_df['package_install_tday'] = app_df.apply(lambda row: [row['packageName'], str(row['installAPP_tday'])], axis=1)
        appNameSentencesDtest = app_df.groupby('loan_app_id')['package_install_tday'].apply(self.__sent_ele).to_dict()
